...
- [[GP-259]] — Orders/Return and COGs for SKU Profitability. Two open issues: (A) dashboard COGS ≠ warehouse — `EXTRACT_SALES_DETAIL` override uses gross qty vs fact's return-adjusted qty; fix = move override upstream to source view (`_gp259_bridge2`); (B) expose `SALES_RETURNS_QUANTITY` for Return Rate KPI. Return logic confirmed live in `SALES_FCT_ORDERLINE` (CCE was wrong — it's warehouse SQL, not DAX/dbt). Deployed state unverified (no Snowflake creds). Updated 2026-06-10.
