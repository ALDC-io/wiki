...
| **Amazon SP-API (Agency)** | LWA OAuth per customer | App registration + per-customer OAuth grants | **Live — new public solution app registered; Lectric eBikes = first agency customer onboarded 2026-05-28** (creds validated US/CA/MX/BR, in all env vaults as `lectric--amazon-spapi--*`). I-1 resolved. Additional agency customers follow the same pattern: own Eclipse conn + own `AMAZON_<ENTITY>` schema, segmented by ENTITY_CODE. See [[navira-dwh-data-landing]]. | 1C |
