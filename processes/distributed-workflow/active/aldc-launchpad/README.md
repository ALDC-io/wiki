...
├── api/credential-exchange/ # Azure Functions (legacy — migrated to prefect-connectors)
...
- 2026-05-22 — Credential metadata store: Postgres (existing Prefect instance `aldcprodpgdbconnector1c01`) over Key Vault tags (limited to 15) or JSON files (no concurrent writes, no indexing). Separate `credentials` schema avoids Prefect table collisions.
- 2026-05-22 — Credential API hosting: `func-aldc-cred` on S1 plan in Production 2 VNet (same subscription as Postgres + Prefect server) over upgraded Consumption plan in Quality 1 (can't do cross-subscription VNet integration)
...
- [[phase-1b-credential-submission]] — Credential exchange API migrated to Postgres-backed lifecycle engine. `/launchpad-phase1b` **Production VNet Deployed 2026-05-22** — 16 endpoints on `func-aldc-cred` (S1 plan, VNet-integrated in Production 2 subscription). Postgres `credentials` schema (5 tables) on `aldcprodpgdbconnector1c01`. Lifecycle engine: auto-validate + auto-provision with CSRF token handling. E2E verified: create → submit → auto-validate → auto-provision → Prefect block created. **Completed.**
- [[phase-1d-automation]] — Reminder chains, health monitoring, Windsor auto-verify. Deferred.

### Completed
- [[phase-1b-credential-submission]] — Credential exchange API: Postgres-backed lifecycle engine (16 endpoints, auto-validate, auto-provision). Migrated from JSON/Consumption plan to `func-aldc-cred` on S1/VNet in Production 2. CSRF + block_schema_id fixes for Prefect 3.x. `/launchpad-phase1b` (2026-05-22)
...
### 2026-05-25 — Orchestrator Phase E: gate approval UI, work pool health, credential pre-flight

- did: Implemented Phase E per [[orchestrator-production-readiness]]. **E1 — Gate approval cards**: enhanced pipeline card gate panels with full Pipeline Context section showing rollback gate status (green/red/amber panels for passed/rolled-back/failed), parity results, flow run metadata, deployment snapshots. Added notes textarea for operator comments. Added Reject button alongside Approve — reject calls new `/api/pipelines/{id}/reject` endpoint. `reject_stage()` in `pipelines.py` marks gate failed and records `gate_rejected` in audit trail. Server emits `pipeline.gate_waiting` SSE event on manual gate dispatch for instant UI updates. **E2 — Work pool health**: `verify_work_pool(pool_name)` in `gate_checks.py` queries Prefect API for pool existence, PAUSED status, and online workers. ACI pools pass with 0 workers (on-demand). Wired as inline pre-flight in `promote_deployment` — promotion fails before creating deployment if pool is unhealthy. **E3 — Credential validation**: `verify_credentials(ctx)` with `_verify_snowflake_target` (SELECT 1) and `_verify_snowflake_source` (GP-261: key-pair auth + SELECT COUNT on each contracted view). Added `preflight-uat-creds` and `preflight-prod-creds` stages to pipeline (21 stages total).
- decided: Work pool check inline in promote_deployment (not a separate pipeline stage) because it's a pre-condition for deployment creation, not a data validation step. Credential checks as separate pipeline stages (preflight-*-creds) because they run after promotion but before trigger — different failure handling than the promotion step itself. ACI pools treated specially: 0 online workers is healthy because ACI workers spin up on-demand when a flow run is submitted.
- commits: `006ff0f` (prefect-connectors: gate_checks + pipelines + server), `505a375` (aldc-launchpad: pipelines.html + submodule)
- status: **All orchestrator hardening phases complete** (0, 0.5, C, D, E). 21-stage connector-promotion pipeline with full rollback mechanism, evidence capture, gate approval context, work pool health checks, and credential pre-flight validation. Ready to test the Definition of Done on Sellercloud (GP-219).
- next: Test the full pipeline on Sellercloud: deploy to QA → validate → promote to UAT → parity → rollback gate → approve → promote to Prod → robustness → approve → schedule. Or build the Navira Snowflake-to-Snowflake connector (GP-261, blocked on Justin's response).

### 2026-05-25 — Orchestrator Phases C + D: rollback mechanism + evidence capture

- did: Implemented Phase C (rollback mechanism) and Phase D (evidence capture) per [[orchestrator-production-readiness]]. **C2 — `rollback_to_snapshot`**: new function in `promotion_ops.py` reads a deployment snapshot JSON (auto-discovers latest by connector/env or accepts explicit path), restores full Prefect deployment config (work pool, work queue, image, entrypoint, tags, parameters) via PATCH API, disables schedule for safety. Helper `_find_latest_snapshot` scans the snapshots directory. **C3 — Rollback gates**: added `uat-rollback-gate` and `prod-rollback-gate` stages to `connector-promotion.yaml` (now 19 stages, was 16). Parity/robustness stages wrapped in gated functions (`run_parity_gated`, `run_robustness_gated`) that always return `success=True` with actual pass/fail in `data.check_passed` — this ensures the pipeline engine dispatches downstream stages regardless of parity outcome. `evaluate_rollback_gate` checks the check result: if passed, auto-advances; if failed, auto-restores from the pre-promotion snapshot and reports. Always succeeds so the manual approval gate fires for operator review. **D2 — Jira evidence comments**: `post_pipeline_evidence` generates structured Atlassian Document Format (ADF) comments with sections for stage results, flow runs, rollback snapshots, rollback events, and issues. Added as `post-evidence` pipeline stage. **D3 — Evidence tab**: new tab in session slideout (appears for pipeline-linked sessions), fetches audit trail JSON, renders summary cards (completed/failed/gates/cost) + vertical timeline with color-coded event cards.
- decided: Gated wrapper pattern (always success=True, pass/fail in data) avoids modifying the pipeline engine's dependency resolution. Rollback gate always succeeds so the manual approval gate fires regardless — operator gets full context to decide. Evidence tab fetches from `data/audits/{pid}.json` (written by audit.py adopted in Phase 0.5).
...
- status: Phases C and D complete. Phases 0, 0.5, A (snowflake_ops), B (promotion_ops), C, D all done. Only Phase E (gate wiring + work pool health checks) remains before the Definition of Done can be tested on Sellercloud.
- next: Phase E — E1 (manual gate approval UI with SSE), E2 (work pool health check), E3 (credential validation). Or test the full pipeline on Sellercloud first.

### 2026-05-25 — Orchestrator Phase 0.5 complete: dashboard split + SSE + YAML pipelines + agent markdown

- did: Completed Phase 0.5 steps 7-10 of the [[orchestrator-production-readiness]] plan. (1) **Dashboard split**: monolithic 4,479-line `orchestrator/index.html` → 7 files: `index.html` (807 lines — SPA shell + router + SSE status bar + inbox sidebar + Zeus Chat panel + modals), `styles/orchestrator.css` (1,500 lines), `scripts/shared.js` (339 lines — state management + utilities + SSE connection + pipeline helpers), `pages/pipelines.html` (137 lines — pipeline cards + DAG + gate actions), `pages/sessions.html` (266 lines — session kanban + slideout), `pages/deploy.html` (98 lines — environment matrix QA/UAT/Prod), `pages/zeus-chat.html` (64 lines — agent dispatch panel). (2) **SSE replaces polling**: removed `setInterval` loops (2s sessions, 5s pipelines), replaced with `EventSource` to `/api/events/stream` with 3s auto-reconnect backoff. (3) **Pipeline YAML extraction**: 5 YAML files (634 lines) at `connectors/orchestrator/pipelines/*.yaml` — connector-migration, credential-provision, data-parity-test, connector-promotion, prefect-infra. `pipelines.py` updated with YAML loader. (4) **Agent markdown extraction**: 5 markdown files (393 lines) at `connectors/orchestrator/agents/*.md` with YAML frontmatter — zeus-review, code-review, user-testing, deploy, refactor. `server.py` updated with agent loader + `/api/agents` + `/api/agents/run` endpoints.
- decided: Dashboard SPA router loads pages into shell container. SSE auto-reconnect at 3s backoff matches Conductor keepalive (25s). Pipeline YAML templates preserve same stage schema as Python definitions. Agent markdown uses YAML frontmatter for metadata (name, type, capabilities) with body as system prompt.
- status: Phase 0.5 fully complete. All 10 steps done. Orchestrator is now modular: engine modules (6 from Conductor), dashboard (7 page files), pipeline definitions (5 YAML), agent definitions (5 markdown). Ready for Phase A (data validation layer).
- next: Phase A — implement `snowflake_ops.py` (compare_row_counts, compare_aggregates, compare_schemas, check_freshness, sample_diff, generate_parity_report). See [[orchestrator-production-readiness]].

### 2026-05-22 — Credential exchange migration Phase C: Postgres + deploy + E2E

- did: Completed credential-exchange migration Phase C (deploy + cutover). **Postgres migration**: ran `001_create_credentials_schema.sql` against `aldcprodpgdbconnector1c01` via Kudu API on Prefect server Web App (bootstrapped pip + psycopg2 on Kudu sidecar since VNet-isolated DB has no public access). Created `credentials` schema with 5 tables (clients, credentials, status_history, provisioning_log, status_definitions). Seeded 1 client (GEP_PREFECT/Navira), 12 credentials, 23 status history entries. **New Function App**: created `func-aldc-cred` in Production 2 subscription on existing S1 App Service Plan (`aldcprodapspprefectserver1c01`) with VNet integration to `prefect-app-service` subnet. Replaced `func-aldc-cred-qa` (Consumption plan, Quality 1 subscription — couldn't reach VNet-isolated Postgres). Set DATABASE_URL, HMAC_SECRET, PREFECT_API_URL, PREFECT_API_AUTH_STRING, KEY_VAULT_URL, Amazon OAuth creds. Managed identity assigned + RBAC Key Vault Secrets Officer on `aldc-cred-vault-qa`. **CSRF fix**: Prefect server has `PREFECT_SERVER_API_CSRF_PROTECTION_ENABLED` — all POST/PATCH requests to Prefect API need CSRF token. Added `_ensure_csrf_token()` to `prefect_blocks.py` (fetches token once, includes `Prefect-Csrf-Token` + `Prefect-Csrf-Client` headers on mutating requests). **block_schema_id fix**: Prefect 3.x requires `block_schema_id` in block document creation payload. Updated `_get_block_type_info()` to also fetch schema via `/block_schemas/filter`. **E2E golden path verified**: create credential → submit via engineer entry → Key Vault store → auto-validate (generic api_key validator) → auto-provision (Prefect Secret block `e2e-gold-test-gep-prefect` created on server) → status `provisioned`. **Portal cutover**: updated API_BASE in `dashboard.html` and `credentials/index.html` to `func-aldc-cred.azurewebsites.net`. Master credentials page now fetches from API first with snake→camelCase key normalization, falls back to static JSON.
- decided: Function App in Production 2 (same subscription/VNet as Postgres + Prefect server) over upgrading QA FA to Premium for cross-subscription VNet. Kudu sidecar as migration runner (bootstrap pip → psycopg2 → execute SQL via Python since psql not available and DB has no public access). Basic auth (not Bearer) is correct for Prefect server behind Caddy reverse proxy.
- commits: `c0df0a8` (aldc-launchpad: portal cutover, 5 files +171/-28), `2f0c0d6` (prefect-connectors: CSRF + schema fix, 1 file +52/-11)
- status: Phase C complete. Credential exchange fully migrated from JSON → Postgres with lifecycle automation. Legacy `api/credential-exchange/` in aldc-launchpad is now superseded (Phase D cleanup pending).
- next: Orchestrator production readiness (Phase A: `snowflake_ops.py` data validation layer). See wiki `orchestrator-production-readiness.md`.
