...
- discoveries surfaced during A.0.6 (PBIX parameterisation):
  ...
  - **Committed credential in PBIX: `CORE_API_CLIENT_TOKEN`.** Value `<REDACTED — core_api MASTER bearer; see vault/infra-credentials.md § MASTER API bearer (default)>` (base64 of `FFFFFFFF0000:<REDACTED — see vault/infra-credentials.md § MASTER API bearer (default)>`) is baked into every dated GEP PBIX in `repos/power_bi/custom/GEP/`. Wiki rule #2 violation — should be vaulted and rotated. Filed to [[potential-tickets]].
