...
>
> **⚠ Don't confuse `eclipse-2.1` with `eclipse_exp` (cost real time, 2026-07-22).** The **current production product** is this `eclipse` repo on its **`eclipse-2.1` branch** (Next 15 App Router, `eclipse.analyticlabs.io`), backed by **`core_api` on *its* `eclipse-2.1` branch + CosmosDB** — the Explorer (dashboards/visuals/dataViews/RBAC) backend lives in `core_api@eclipse-2.1` (`v1/route_explorer.py`, `api/dashboards|visuals|data_views`, `api/permission_checker.py`), **not** in `eclipse_exp`. `eclipse_exp` is a *separate* Postgres-backed successor for **new** tenants; do **not** read it to answer eclipse-2.1 Explorer/data-isolation questions. When investigating, check out/read the **`eclipse-2.1` branch of BOTH repos**. Full map: [[core_api]] § "Eclipse-2.1 Explorer backend".
>
