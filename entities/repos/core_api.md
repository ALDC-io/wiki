...
- **Mode 2 (None crash) — ⛔ NOT FIXED IN PROD. The fix is written but has never merged.** Corrected 2026-09-08; this bullet previously said "FIXED 2026-07-22" and that was wrong. core_api PR [#246](https://github.com/ALDC-io/core_api/pull/246) (`fix/dataset-sync-cardinality-none-guard`, +4/−1, opened 2026-07-22) is **still OPEN after 48 days, and its base is `main`** — the stale lineage, not the `eclipse-2.1` trunk. Measured 2026-09-08: `git show origin/main:v1/route_dataset.py` → line 1280 `if column_cardinality <= 100:` and `git show origin/eclipse-2.1:v1/route_dataset.py` → line 1267, the same unguarded line. **The guard is on neither trunk, so the crash is still live.** The intended change is `if column_cardinality is not None and column_cardinality <= 100`, validated in-process at the time against TEST `da8904db` "Sales Model" (e80ffd34) + "Marshall Test" (0025d4b4), PBI `66151728` → 17 tables / 116 cols / 86 measures each, no crash. **To actually ship it:** re-target the PR onto `eclipse-2.1` (do not merge to `main`), then it reaches prod on the next **manual** v1 Function App publish — `v1/` has no CI/CD, and the `Dockerfile` copies only `common/` and `api/`, so a container deploy will not carry it. Tracked as GP-302.
