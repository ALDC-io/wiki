...
- **Flight Check sync `only_recent` window silently drops flights** ([[FU92-419]], found 2026-06-05). `dax_api/sync/lib.py` `RECENT_DAYS_TO_PULL = 30`; the timers call `sync_flight_check_flights(only_recent=True)` → pulls only Cosmos docs with `_ts` in the last 30 days. If the sync is down for a stretch and a flight was edited just before the outage, on resume it has aged out of the window and is **never re-synced** — its stale CDC snapshot persists forever. Caused 6T901's "$0 / no data" (Job PRJ002345): `_ts`=2026-05-04 fell 1 day outside the window when the sync recovered ~06/03. Fix = periodic full reconcile (`only_recent=False`) or a last-successful-sync high-water-mark. Un-stuck operationally 2026-06-05 (re-trigger sync + refresh dynamic-table chain → 6T901 = $89,090.91); the durable code fix is FU92-419.
- **Legacy Firebase Flight Check sync still scheduled** ([[FU92-419]]). The pre-Nov-2024 Firebase app (project `flight-check-ae37d`) was replaced by the Cosmos/DAX app, but an Eclipse connector still syncs its ~10,444 dead flights into `FLIGHT_CHECK.FLIGHTS_FLIGHTS` and fires "Urgent — customer impacted" alerts on failure (for data nobody uses). Retire it (confirm no consumer reads `FLIGHT_CHECK.FLIGHTS_FLIGHTS` / the `LEGACY_FLIGHT_CHECK_FLIGHTS` view first).
